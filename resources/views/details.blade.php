...
<main class="pt-90">
    ...
    <!-- Display validation errors above the product details -->
    @if ($errors->any())
            ...
            <div class="col-lg-7">
                <div class="product-single__media" data-media-type="vertical-thumbnail">
                        ...
                        <div class="swiper-container">
                            <div class="swiper-wrapper">
                                ...
                                @foreach (explode(",", $product->images) as $gimg)
                                    <div class="swiper-slide product-single__image-item">
                                        <img loading="lazy" class="h-auto" src="{{ asset('uploads/products') }}/{{ $gimg }}" width="674" height="674" alt="testtesttest" />
                    ...
                    <div class="product-single__thumbnail">
                        <div class="swiper-container">
                            <div class="swiper-wrapper">
                                <div class="swiper-slide product-single__image-item"><img loading="lazy" class="h-auto" src="{{ asset('uploads/products/thumbnail') }}/{{ $product->image }}" width="104" height="104" alt="" /></div>
                                @foreach (explode(",", $product->images) as $gimg)
                                    <div class="swiper-slide product-single__image-item"><img loading="lazy" class="h-auto" src="{{ asset('uploads/products/thumbnail') }}/{{ $gimg }}" width="104" height="104" alt="" /></div>
                                @endforeach
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-lg-5">
                ...
                <div class="product-single__addtolinks">
                    ...
                    <form action="{{route('wishlist.remove', ['rowId'=>Cart::instance('wishlist')->content()->where('id',$product->id)->first()->rowId])}}" method="POST" id="frm-remove-item">
                        @csrf
                        @method('DELETE')
                    <a href="javascript:void(0)" class="menu-link menu-link_us-s add-to-wishlist text-red" onclick="document.getElementById('frm-remove-item').submit()"><svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><use href="#icon_heart" />
                    </svg>
                    <span>
                    ...
                    <form action="{{route('wishlist.add')}}" method="POST" id="wishlist-form">
                        ...
                        <a href="javascript::void(0)" onclick="getElementById('wishlist-form').submit()" class="menu-link menu-link_us-s add-to-wishlist"><svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><use href="#icon_heart" />
                            </svg>
                        ...
                        <div class="swiper-slide product-card">
                            <div class="pc__img-wrapper">
                                <a href="{{ route('shop.product.details', ['product_slug' => $rproduct->slug]) }}">
                                    <img loading="lazy" class="pc__img pc__img--primary" src="{{ $rproduct->image ? asset('uploads/products/' . explode(',', $rproduct->image)[0]) : asset('images/placeholder.jpg') }}" width="270" height="270" alt="{{ $rproduct->name }}" />
                                    @if($rproduct->image)
                                        @foreach (explode(',', $rproduct->image) as $index => $gimg)
                                            @if($index > 0)
                                                <img loading="lazy" class="pc__img pc__img--gallery" src="{{ asset('uploads/products/' . $gimg) }}" width="270" height="270" alt="{{ $rproduct->name }}" style="display: none;" />
                                            @endif
                                        @endforeach
                                    @endif
                                </a>
                                <!-- Add a form for the "Add To Cart" button -->
                                <form action="{{ route('cart.add') }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="id" value="{{ $rproduct->id }}">
                                    <input type="hidden" name="name" value="{{ $rproduct->name }}">
                                    <input type="hidden" name="price" value="{{ empty($rproduct->sale_price) ? ($rproduct->normal_price ?: 0) : ($rproduct->sale_price ?: 0) }}">
                                    <input type="hidden" name="quantity" value="1">
                                ...
                                <div class="product-card__price d-flex">
                                    <span class="money price">
                                        @if($rproduct->sale_price)
                                            <s>${{ $rproduct->normal_price }}</s> ${{ $rproduct->sale_price }}
                                        @else
                                            ${{ $rproduct->normal_price }}
                                        @endif
                                    </span>
                                </div>
